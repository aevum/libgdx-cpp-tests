...
class SpriteBatchTest : public gdx::ApplicationListener {
public:
    ...
    void create() {        
        ...
        gdx::FileHandle::ptr file = gdx::files->internal("data/badlogicsmall.jpg");
        Pixmap::ptr pixmap = Pixmap::newFromFile(file);

        texture = Texture::ptr(new Texture(32, 32, gdx::Pixmap::Format::RGB565, Pixmap::Gdx2d));
    ...
    void renderNormal() {
        ...
        if (gdx::system->nanoTime() - startTime > 1000000000) {
            gdx_log_debug("SpriteBatch", "fps: %d , render calls: %d, begin: %d us,"
...
//         draw2 = (gdx::system->nanoTime() - start) / 1000000000.0f;
        ...
        if (gdx::system->nanoTime() - startTime > 1000000000) {
            gdx_log_debug("SpriteBatch", "fps: %d , render calls: %d, %f, %f ,%f, %f, %f", frames, spriteBatch->renderCalls, frames,  begin,
            draw1, draw2, drawText, end);
            frames = 0;
            startTime = gdx::system->nanoTime();
        }
